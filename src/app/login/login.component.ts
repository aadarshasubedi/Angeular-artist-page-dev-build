...
error;
artistName;
  constructor(private http: HttpClient, private router:Router, private user: UserService, private menu: MenuComponent) {
...
console.log("User is logged in? ", this.user.getUserLoggedIn());
  ...
  loginUser(e) {
    ...
    console.log("encoded token " + encodedToken);
      ...
      const body = {/*"grant_type":"password", "username": usernameInput, "password": passwordInput */};
    const req = this.http.post('https://s-test.wr1.com/oauth/token', body, { params: new HttpParams().set('grant_type','password').set('username',usernameInput).set('password',passwordInput),
       headers: new HttpHeaders().set('Authorization','Basic '+ encodedToken).set('Content-Type','application/x-www-form-urlencoded'),  // put headers here
    });
    console.log(body)
    req.subscribe(
